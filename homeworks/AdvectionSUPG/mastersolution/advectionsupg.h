...
SUAdvectionElemMatrixProvider<MESHFUNCTION_V>::Eval(const lf::mesh::Entity &cell) {
  ...
  for(unsigned i = 0; i < 6; i++){
      const double fac = qr.Weights()*dets[i];      
      const Eigen::Matrix <double , 2 , 6> TrfG = JinvT.block(0, 2*i, 2, 2) * (grad_ref_lsf_.block(0, 2*i, 6, 2).transpose());
  }
  // ========================================
