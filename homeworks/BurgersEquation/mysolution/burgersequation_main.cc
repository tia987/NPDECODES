...
int main() {
    ...
    //====================
    std::ofstream sol;
    sol.open("solution.csv");
    sol << x.transpose().format(BurgersEquation::CSVFormat) << std::endl;
    sol << mu03.transpose().format(BurgersEquation::CSVFormat) << std::endl;
    sol << mu30.transpose().format(BurgersEquation::CSVFormat) << std::endl;
    sol.close();
