...
std::vector<Eigen::Triplet<double>> computeA(const Eigen::VectorXd &mesh,
    ...
    for(unsigned i = 0; i < N; i++){ 
        if(i == 0){
            triplets.push_back(Eigen::Triplet<double>(i,i,1/(mesh(i+1)-mesh(i))+1/(mesh(i+2)-mesh(i+1))));
            triplets.push_back(Eigen::Triplet<double>(i,i+1,-1/(mesh(i+1)-mesh(i))));

        }
        else if(i == N-1){
          triplets.push_back(Eigen::Triplet<double>(i,i,1/(mesh(i-1)-mesh(i-2))+1/(mesh(i)-mesh(i-1))));
            triplets.push_back(Eigen::Triplet<double>(i,i-1,-1/(mesh(i+1)-mesh(i))));
        }
        else {
          triplets.push_back(Eigen::Triplet<double>(i,i,1/(mesh(i)-mesh(i-1))+1/(mesh(i+1)-mesh(i))));
          triplets.push_back(Eigen::Triplet<double>(i,i-1,-1/(mesh(i)-mesh(i-1))));
          triplets.push_back(Eigen::Triplet<double>(i,i+1,-1/(mesh(i+1)-mesh(i))));
        }        
    }
    //====================
...
std::vector<Eigen::Triplet<double>> computeM(const Eigen::VectorXd &mesh,
    ...
    //====================
    for(unsigned i = 0; i < N; i++){        
      triplets.push_back(Eigen::Triplet<double>(i,i,std::abs(gamma(mesh(i))*mesh(i+1)+mesh(i))*0.5));
    }
    //====================
...
Eigen::VectorXd solveA(const Eigen::VectorXd &mesh, FUNCTOR1 &&gamma,
    ...
    //====================
    auto triplets = computeA(mesh, gamma);
    A.setFromTriplets(triplets.begin(), triplets.end()); 
    triplets = computeM(mesh, gamma);
    ...
    //====================
    Eigen::VectorXd b = computeRHS(mesh,f);
    //====================
